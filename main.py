...
def Geenrate_MIS_Report():   
   ...
   if RM_name == 'all':
     smallcase_clients = Smallcase_data[Smallcase_data['RM'].isin(['rahul m v', 'mudit', 'chandan br', 'arun mathew','ashish lal','binto sebastian', 'manju - suhas','manju - rahul', 'khushboo sheth','manju mastakar'])]
     vested_clients = VESTED_data.loc[VESTED_data['RM'].isin(['rahul m v', 'mudit', 'chandan br', 'arun mathew', 'ashish lal', 'binto sebastian', 'manju - suhas','manju - rahul', 'khushboo sheth','manju mastakar'])]
     filtered_df = master_data[(master_data['RM Name'].isin(['rahul m v', 'mudit', 'chandan br', 'arun mathew', 'ashish lal', 'binto sebastian', 'manju - suhas','manju - rahul', 'khushboo sheth','manju mastakar']))]  
   ...
   def filter_smallcase_clients(smallcase_clients: pd.DataFrame, timeperiod, selected_month = None, selected_quarter_fy = None,selected_cy= None,selected_fy=None) -> pd.DataFrame:
    ...
    filtered_df_smallcase = filtered_df_smallcase.rename(columns={'Name': 'NAME', 'Smallcase Name': 'SMALLCASE NAME','Networth': 'CURRENT VALUE','Subscription Start Date':'SUBSCRIPTION START DATE'})
   ...
   def filter_vested_clients(vested_clients: pd.DataFrame, timeperiod, selected_month = None, selected_quarter_fy = None,selected_cy=None,selected_fy=None) -> pd.DataFrame:
    ...
    columns_to_select_vested = ['Name', 'Plan Type', 'Current Value', 'Invested Amount','RC Date','RM']
    ...
    filtered_df_vested['RM'] = filtered_df_vested['RM'].str.upper()   
   ...
   if st.button("Generate Simple PDF Report"):
      with st.spinner("Generating..."):
          ...
          pdf_path = create_simple_investment_report(selected_month,investment_df,rm_name,filtered_smallcase_df,
               filtered_vested_df,filtered_pms_df,filtered_bonds_df,filtered_fd_df,filtered_aif_df,filtered_bank_df,filtered_insurance_df, filtered_F_real_estate_df,output_path=temp_path)
