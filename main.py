...
def Geenrate_MIS_Report():   
   ...
   def filter_fd_clients(FD_clients: pd.DataFrame, timeperiod, selected_month = None, selected_quarter_fy = None,selected_cy=None,selected_fy=None) -> pd.DataFrame:
    ...
    filtered_df_fd = filtered_df_fd.rename({'Name':'NAME','Issue Date':'ISSUE DATE', 'Channel Partner':'CHANNEL PARTNER','Investment Amount':'INVESTED AMOUNT'})
   ...
   if st.button("Generate Simple PDF Report"):
      with st.spinner("Generating..."):
            ...
            pdf_path = create_simple_investment_report(investment_df,rm_name,selected_month,filtered_smallcase_df,
               filtered_vested_df,filtered_pms_df,filtered_bonds_df,filtered_df_fd,output_path=temp_path)
